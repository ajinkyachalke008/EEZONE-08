...
async function captureClassic() {
  ...
  console.log('Capturing Home screen classic cards (/)...');
  ...
  await page.screenshot({ path: path.join(ARTIFACT_DIR, '33_all_cards_classic_gradient_live.png') });
  console.log('✓ Saved 33_all_cards_classic_gradient_live.png');
  ...
  await new Promise(r => setTimeout(r, 1200));
  await page.screenshot({ path: path.join(ARTIFACT_DIR, '34_master_labs_classic_gradient_live.png') });
  console.log('✓ Saved 34_master_labs_classic_gradient_live.png');

  await browser.close();
  console.log('=== CLASSIC CARDS CAPTURED ===');
}
